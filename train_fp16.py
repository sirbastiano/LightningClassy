################################################################################################
# 1. Import libraries and modules, define constants and inizialization
################################################################################################
...
# Precision and seed setting, for matrix multiplication operations in PyTorch and for all random number generators respectively
torch.set_float32_matmul_precision('medium')  # For performance
seed = pl.seed_everything(42, workers=True) # For reproducibility

# Initialize a CSVLogger from PyTorch Lightning, which will record training progress and save it to a CSV file for later analysis
home = '/Data/marine/Datasets/VENuS/classification/perfect/'
csv_logger = CSVLogger('.', name='/Data/marine/Datasets/VENuS/classification/perfect/lightning_logs/vessels/fp16')
...
efficientnet_lite_params = {
    # model_name = [width_coefficient, depth_coefficient, image_size, dropout_rate]
...
## WHEN SCALING THE WIDTH OF THE MODEL:
# The round_filters function adjust the number of filters based on the width multiplier, 
# ensuring that the new number of filters is divisible by the "divisor" and does not decrease more than 10%.
...
## WHEN SCALING THE DEPTH OF THE MODEL:
# The round_repeats function adjusts the number of repeats (depth of the network) based on the depth multiplier. 
...
## TO HELP TO PREVENT OVERFITTING
# The drop_connect function implements a regularization technique called “DropConnect”, which is similar to Dropout, 
# but drops out entire connections instead of individual neurons during training
...
# The MoBile inverted bottleneck Convolution (MBConv) block is a key component of the EfficientNet architecture.
class MBConvBlock(nn.Module):
    '''
    The __init__ method initializes an instance of the MBConvBlock class. Here's a breakdown of its parameters:
    - inp: The number of input channels.
    - final_oup: The number of output channels after the final 1x1 convolution.
    - k: The kernel size for the depthwise convolution.
    - s: The stride for the depthwise convolution.
    - expand_ratio: The expansion factor for the initial 1x1 convolution. If this is not 1, the number of channels is increased by this factor.
    - se_ratio: The ratio for the squeeze-and-excitation operation. This determines the number of channels after the squeeze operation.
    - has_se: A boolean flag indicating whether to include a squeeze-and-excitation operation.

    The class also defines several layers used in the block:
    - _expand_conv: A 1x1 convolution layer for channel expansion.
    - _bn0, _bn1, _bn2: Batch normalization layers for normalization.
    - _depthwise_conv: A depthwise convolution layer.
    - _se_reduce, _se_expand: Convolution layers for the squeeze-and-excitation operation.
    - _project_conv: A 1x1 convolution layer for channel projection.
    - _relu: A ReLU6 activation function.
    '''
    ...
    def forward(self, x, drop_connect_rate=None):
        ...
        The forward method of the MBConvBlock class describes how the input tensor x is transformed as it passes through the block. 
        It applies the layers defined in the __init__ method in a specific order, with some layers being conditionally applied based on the expand_ratio and has_se parameters:
        1. Expansion and Depthwise Convolution: If expand_ratio is not 1, the number of channels in x is first increased using the _expand_conv layer, 
           followed by batch normalization and ReLU6 activation. Then, a depthwise convolution is applied to x, followed by another batch normalization and ReLU6 activation.
        2. Squeeze and Excitation: If has_se is True, a squeeze-and-excitation operation is performed. This operation first applies global average pooling to x to squeeze it 
           into a tensor with 1x1 spatial dimensions. Then, it reduces the number of channels using the _se_reduce layer, applies ReLU6 activation, expands the number of channels back 
           to the original number using the _se_expand layer, and applies a sigmoid activation. The output is then multiplied with the original x to recalibrate the channel-wise feature responses.
        3. Output Phase: A 1x1 convolution is applied to x to project the number of channels to final_oup, followed by batch normalization.
        4. Skip Connection and Drop Connect: If id_skip is True, the stride is 1, and the number of input and output filters are the same, a skip connection is added. 
           This involves adding the original input tensor (identity) to x. If drop_connect_rate is provided, a DropConnect operation is applied to x before adding the skip connection.
           
        The output of the block is the final x tensor. This block is a fundamental building block of the EfficientNet architecture and is repeated multiple times with different 
        parameters to form the complete model. The squeeze-and-excitation operation and the skip connection are key features of this block that help improve the model's performance. 
        The squeeze-and-excitation operation allows the model to adaptively recalibrate channel-wise feature responses, and the skip connection helps alleviate the vanishing gradient 
        problem during training. The DropConnect operation is a regularization technique that helps prevent overfitting. The expansion phase and the depthwise convolution help increase 
        the model's capacity without a significant increase in computational cost. The output phase reduces the dimensionality of the feature maps before they are passed to the next block. 
        The use of ReLU6 activation functions and batch normalization also contribute to the model's performance. The exact impact of these components can vary depending on the specific task and dataset. 
        """
...
class VenusClassifier(pl.LightningModule):
    def __init__(self, image_dir='.', batch_size=32, lr=1e-3,image_size=(128, 128), train_split=0.5, val_split=0.3):
        ...
        #### Inserted because the it was seen 0 categories insted of 5
        num_classes = len(categories)
        print(f"num cat: {num_classes}")
        #############################
        ...
        self.f1_score = torchmetrics.classification.f_beta.F1Score(num_classes=self.num_classes, average='macro', task='multiclass')
        ...
        self.balanced_accuracy = torchmetrics.Accuracy(average='macro', num_classes=self.num_classes, task='multiclass')
...
if __name__ == '__main__':
    ...
    Pr = args.precision
    # Start training
    venus_classifier = VenusClassifier(image_dir=f'{home}/V2C/128x128/5_cat_aug', batch_size=BS, image_size=(128, 128), train_split=0.5, val_split=0.3, lr=LR)
    ...
    save_metrics_to_csv(test_results, f'{home}/test_results/vessels/fp16_venus_classifier_{BS}_{LR}.csv')
